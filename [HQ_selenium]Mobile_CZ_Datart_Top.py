...
d1 = {
    ...
    #'Path' : sys.argv[2],
    'Name_Class' : "top_item_detail",
    'Top_Tile' : "top_ten_items",
    'Price' : "actual",
    'Price_Class' : "tooltip",
    }
...
def get_price(top_tile, price, price_class):
    data_text = []
    prd_tile = soup.find_all("div", {"id":top_tile})[1]
    list_data = list(prd_tile.find_all(class_=price))
    for x in list_data:
        text = x.find(class_=price_class).get_text().strip().replace('Kč','').replace(" ", "")
        data_text.append(text)
    return data_text
